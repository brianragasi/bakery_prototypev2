...
$redemptionRate = 0.01; // Example redemption rate: 1 point = $0.01
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['place_order'])) {
    ...
    if ($orderType === 'delivery' && empty(trim($address))) {
        ...
        $order = new Order();
        // Use $_SESSION['user_email'] (set during login) for customer email
        ...
        if (strpos($orderId, 'Error:') === 0) {
            ...
            // Deduct loyalty points after successful order
        ...
        <section class="checkout bg-white p-8 rounded-lg shadow-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="order-summary">
                    ...
                    <p class="text-lg font-bold mt-4">Total: $<?= number_format($cartTotal, 2); ?></p>

                    <!-- Loyalty Points Section (if applicable) -->
                    <?php if ($points > 0): ?> 
                        <form method="post" class="mt-6 space-y-4">
                            <label for="redeem_points" class="block text-gray-700 font-bold">Redeem Loyalty Points 
                                (<?= $points ?> available, 1 point = $<?= $redemptionRate ?>):
                            </label>
                            <input type="number" id="redeem_points" name="redeem_points" 
                                value="<?= $pointsToRedeem ?>" min="0" max="<?= $maxPoints ?>"
                                class="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-primary"
                            >
                            <button type="submit" class="bg-primary text-white font-bold py-2 px-4 rounded hover:bg-green-600 transition-colors">Apply</button>
                        </form>

                        <?php if (isset($_POST['redeem_points'])): ?> 
                            <p class="mt-2">Discount: $<?= number_format($discount, 2); ?></p> 
                            <p class="text-lg font-bold mt-2">Final Total: $<?= number_format($finalTotal, 2); ?></p>
                        <?php endif; ?> 
                    <?php endif; ?>
                </div> 
